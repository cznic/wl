// Code generated by yy. DO NOT EDIT.

// Copyright 2017 The WL Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

package wl

import (
	"fmt"
)

func ExampleCommaOpt() {
	fmt.Println(exampleAST(122, "1.97 [ 1.98 ]") == (*CommaOpt)(nil))
	// Output:
	// false
}

func ExampleCommaOpt_case1() {
	fmt.Println(exampleAST(123, "{ 1.97 , ]"))
	// Output:
	// &wl.CommaOpt{
	// · Token: ',',
	// }
}

func ExampleExprList() {
	fmt.Println(exampleAST(120, "{ 1.97 ,"))
	// Output:
	// &wl.ExprList{
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// }
}

func ExampleExprList_case1() {
	fmt.Println(exampleAST(121, "{ 1.97 , 1.98 ,"))
	// Output:
	// &wl.ExprList{
	// · ExprList: &wl.ExprList{
	// · · Case: 1,
	// · · Expression: &wl.Expression{
	// · · · Case: 55,
	// · · · Factor: &wl.Factor{
	// · · · · Term: &wl.Term{
	// · · · · · Case: 4,
	// · · · · · Token: FLOAT, "1.98",
	// · · · · },
	// · · · },
	// · · },
	// · · Token: ',',
	// · },
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// }
}

func ExampleExpression() {
	fmt.Println(exampleAST(2, "++ 1.97"))
	// Output:
	// &wl.Expression{
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: INC, "++",
	// }
}

func ExampleExpression_case01() {
	fmt.Println(exampleAST(3, "-- 1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 1,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: DEC, "--",
	// }
}

func ExampleExpression_case02() {
	fmt.Println(exampleAST(4, "\\@ 1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 2,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: SQRT2, "\\@",
	// }
}

func ExampleExpression_case03() {
	fmt.Println(exampleAST(5, "\\@ 1.97 \\% 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 3,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: SQRT2, "\\@",
	// · Token2: POWER_SUBSCRIPT2, "\\%",
	// }
}

func ExampleExpression_case04() {
	fmt.Println(exampleAST(6, "\\[Sqrt] 1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 4,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: SQRT, "√",
	// }
}

func ExampleExpression_case05() {
	fmt.Println(exampleAST(7, "\\[Integrate] 1.97 \\[DifferentialD] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 5,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: INTEGRATE, "∫",
	// · Token2: DIFFERENTIAL_D, "\uf74c",
	// }
}

func ExampleExpression_case06() {
	fmt.Println(exampleAST(8, "! 1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 6,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: '!',
	// }
}

func ExampleExpression_case07() {
	fmt.Println(exampleAST(9, "- 1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 7,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: '-',
	// }
}

func ExampleExpression_case08() {
	fmt.Println(exampleAST(10, "1.97 && 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 8,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: AND, "&&",
	// }
}

func ExampleExpression_case09() {
	fmt.Println(exampleAST(11, "1.97 ++"))
	// Output:
	// &wl.Expression{
	// · Case: 9,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: INC, "++",
	// }
}

func ExampleExpression_case10() {
	fmt.Println(exampleAST(12, "1.97 --"))
	// Output:
	// &wl.Expression{
	// · Case: 10,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: DEC, "--",
	// }
}

func ExampleExpression_case11() {
	fmt.Println(exampleAST(13, "1.97 -> 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 11,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: RULE, "->",
	// }
}

func ExampleExpression_case12() {
	fmt.Println(exampleAST(14, "1.97 /* 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 12,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: RIGHT_COMPOSITION, "/*",
	// }
}

func ExampleExpression_case13() {
	fmt.Println(exampleAST(15, "1.97 /. 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 13,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: REPLACEALL, "/.",
	// }
}

func ExampleExpression_case14() {
	fmt.Println(exampleAST(16, "1.97 // 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 14,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: POSTFIX, "//",
	// }
}

func ExampleExpression_case15() {
	fmt.Println(exampleAST(17, "1.97 //. 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 15,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: REPLACEREP, "//.",
	// }
}

func ExampleExpression_case16() {
	fmt.Println(exampleAST(18, "1.97 //@ 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 16,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: MAP_ALL, "//@",
	// }
}

func ExampleExpression_case17() {
	fmt.Println(exampleAST(19, "1.97 /; 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 17,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: CONDITION, "/;",
	// }
}

func ExampleExpression_case18() {
	fmt.Println(exampleAST(20, "1.97 /@ 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 18,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: MAP, "/@",
	// }
}

func ExampleExpression_case19() {
	fmt.Println(exampleAST(21, "1.97 := 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 19,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: SET_DELAYED, ":=",
	// }
}

func ExampleExpression_case20() {
	fmt.Println(exampleAST(22, "1.97 :> 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 20,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: RULEDELAYED, ":>",
	// }
}

func ExampleExpression_case21() {
	fmt.Println(exampleAST(23, "1.97 <= 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 21,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: LEQ, "<=",
	// }
}

func ExampleExpression_case22() {
	fmt.Println(exampleAST(24, "1.97 <> 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 22,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: STRINGJOIN, "<>",
	// }
}

func ExampleExpression_case23() {
	fmt.Println(exampleAST(25, "1.97 =!= 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 23,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: UNSAME, "=!=",
	// }
}

func ExampleExpression_case24() {
	fmt.Println(exampleAST(26, "1.97 == 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 24,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: EQUAL, "==",
	// }
}

func ExampleExpression_case25() {
	fmt.Println(exampleAST(27, "1.97 === 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 25,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: SAME, "===",
	// }
}

func ExampleExpression_case26() {
	fmt.Println(exampleAST(28, "1.97 >= 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 26,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: GEQ, ">=",
	// }
}

func ExampleExpression_case27() {
	fmt.Println(exampleAST(29, "1.97 @* 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 27,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: COMPOSITION, "@*",
	// }
}

func ExampleExpression_case28() {
	fmt.Println(exampleAST(30, "1.97 @@ 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 28,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: APPLY, "@@",
	// }
}

func ExampleExpression_case29() {
	fmt.Println(exampleAST(31, "1.97 @@@ 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 29,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: APPLY_ALL, "@@@",
	// }
}

func ExampleExpression_case30() {
	fmt.Println(exampleAST(32, "1.97 \\& 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 30,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: OVERSCRIPT, "\\&",
	// }
}

func ExampleExpression_case31() {
	fmt.Println(exampleAST(33, "1.97 \\+ 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 31,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: UNDERSCRIPT, "\\+",
	// }
}

func ExampleExpression_case32() {
	fmt.Println(exampleAST(34, "1.97 \\^ 1.98 \\% 1.99"))
	// Output:
	// &wl.Expression{
	// · Case: 32,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Expression3: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.99",
	// · · · },
	// · · },
	// · },
	// · Token: POWER_SUBSCRIPT1, "\\^",
	// · Token2: POWER_SUBSCRIPT2, "\\%",
	// }
}

func ExampleExpression_case33() {
	fmt.Println(exampleAST(35, "1.97 \\_ 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 33,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: SUBSCRIPT, "\\_",
	// }
}

func ExampleExpression_case34() {
	fmt.Println(exampleAST(36, "1.97 || 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 34,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: OR, "||",
	// }
}

func ExampleExpression_case35() {
	fmt.Println(exampleAST(37, "1.97 * 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 35,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '*',
	// }
}

func ExampleExpression_case36() {
	fmt.Println(exampleAST(38, "1.97 + 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 36,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '+',
	// }
}

func ExampleExpression_case37() {
	fmt.Println(exampleAST(39, "1.97 - 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 37,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '-',
	// }
}

func ExampleExpression_case38() {
	fmt.Println(exampleAST(40, "1.97 . 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 38,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '.',
	// }
}

func ExampleExpression_case39() {
	fmt.Println(exampleAST(41, "1.97 / 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 39,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '/',
	// }
}

func ExampleExpression_case40() {
	fmt.Println(exampleAST(42, "1.97 : 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 40,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: ':',
	// }
}

func ExampleExpression_case41() {
	fmt.Println(exampleAST(43, "1.97 ;"))
	// Output:
	// &wl.Expression{
	// · Case: 41,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: ';',
	// }
}

func ExampleExpression_case42() {
	fmt.Println(exampleAST(44, "1.97 ; 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 42,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: ';',
	// }
}

func ExampleExpression_case43() {
	fmt.Println(exampleAST(45, "1.97 < 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 43,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '<',
	// }
}

func ExampleExpression_case44() {
	fmt.Println(exampleAST(46, "1.97 = 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 44,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '=',
	// }
}

func ExampleExpression_case45() {
	fmt.Println(exampleAST(47, "1.97 > 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 45,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '>',
	// }
}

func ExampleExpression_case46() {
	fmt.Println(exampleAST(48, "1.97 ? 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 46,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '?',
	// }
}

func ExampleExpression_case47() {
	fmt.Println(exampleAST(49, "1.97 @ 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 47,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '@',
	// }
}

func ExampleExpression_case48() {
	fmt.Println(exampleAST(50, "1.97 ^ 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 48,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '^',
	// }
}

func ExampleExpression_case49() {
	fmt.Println(exampleAST(51, "1.97 | 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 49,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '|',
	// }
}

func ExampleExpression_case50() {
	fmt.Println(exampleAST(52, "1.97 ~ 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 50,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: '~',
	// }
}

func ExampleExpression_case51() {
	fmt.Println(exampleAST(53, "1.97 \\[Conjugate]"))
	// Output:
	// &wl.Expression{
	// · Case: 51,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: CONJUGATE, "\uf3c8",
	// }
}

func ExampleExpression_case52() {
	fmt.Println(exampleAST(54, "1.97 \\[ConjugateTranspose]"))
	// Output:
	// &wl.Expression{
	// · Case: 52,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: CONJUGATE_TRANSPOSE, "\uf3c9",
	// }
}

func ExampleExpression_case53() {
	fmt.Println(exampleAST(55, "1.97 \\[HermitianConjugate]"))
	// Output:
	// &wl.Expression{
	// · Case: 53,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: HERMITIAN_CONJUGATE, "\uf3ce",
	// }
}

func ExampleExpression_case54() {
	fmt.Println(exampleAST(56, "1.97 \\[Transpose]"))
	// Output:
	// &wl.Expression{
	// · Case: 54,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: TRANSPOSE, "\uf3c7",
	// }
}

func ExampleExpression_case55() {
	fmt.Println(exampleAST(57, "1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 55,
	// · Factor: &wl.Factor{
	// · · Term: &wl.Term{
	// · · · Case: 4,
	// · · · Token: FLOAT, "1.97",
	// · · },
	// · },
	// }
}

func ExampleExpression_case56() {
	fmt.Println(exampleAST(58, "1.97 \\[PartialD] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 56,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: PARTIAL_D, "∂",
	// }
}

func ExampleExpression_case57() {
	fmt.Println(exampleAST(59, "\\[Del] 1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 57,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: DEL, "∇",
	// }
}

func ExampleExpression_case58() {
	fmt.Println(exampleAST(60, "1.97 \\[DiscreteShift] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 58,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: DISCRETE_SHIFT, "\uf4a3",
	// }
}

func ExampleExpression_case59() {
	fmt.Println(exampleAST(61, "1.97 \\[DiscreteRatio] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 59,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: DISCRETE_RATIO, "\uf4a4",
	// }
}

func ExampleExpression_case60() {
	fmt.Println(exampleAST(62, "1.97 \\[DifferenceDelta] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 60,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: DIFFERENCE_DELTA, "∆",
	// }
}

func ExampleExpression_case61() {
	fmt.Println(exampleAST(63, "\\[Square] 1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 61,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: SQUARE, "\uf520",
	// }
}

func ExampleExpression_case62() {
	fmt.Println(exampleAST(64, "1.97 \\[SmallCircle] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 62,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: SMALL_CIRCLE, "∘",
	// }
}

func ExampleExpression_case63() {
	fmt.Println(exampleAST(65, "1.97 \\[CircleDot] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 63,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: CIRCLE_DOT, "⊙",
	// }
}

func ExampleExpression_case64() {
	fmt.Println(exampleAST(66, "1.97 ** 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 64,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: NON_COMMUTATIVE_MULTIPLY, "**",
	// }
}

func ExampleExpression_case65() {
	fmt.Println(exampleAST(67, "1.97 \\[Cross] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 65,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: CROSS, "\uf4a0",
	// }
}

func ExampleExpression_case66() {
	fmt.Println(exampleAST(68, "+ 1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 66,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: '+',
	// }
}

func ExampleExpression_case67() {
	fmt.Println(exampleAST(69, "\\[PlusMinus] 1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 67,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: PLUS_MINUS, "±",
	// }
}

func ExampleExpression_case68() {
	fmt.Println(exampleAST(70, "\\[MinusPlus] 1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 68,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: MINUS_PLUS, "∓",
	// }
}

func ExampleExpression_case69() {
	fmt.Println(exampleAST(71, "1.97 \\[Backslash] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 69,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: BACKSLASH, "∖",
	// }
}

func ExampleExpression_case70() {
	fmt.Println(exampleAST(72, "1.97 != 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 70,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: UNEQUAL, "!=",
	// }
}

func ExampleExpression_case71() {
	fmt.Println(exampleAST(73, "1.97 \\[Diamond] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 71,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: DIAMOND, "⋄",
	// }
}

func ExampleExpression_case72() {
	fmt.Println(exampleAST(74, "1.97 \\[Wedge] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 72,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: WEDGE, "⋀",
	// }
}

func ExampleExpression_case73() {
	fmt.Println(exampleAST(75, "1.97 \\[Vee] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 73,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: VEE, "⋁",
	// }
}

func ExampleExpression_case74() {
	fmt.Println(exampleAST(76, "1.97 \\[CircleTimes] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 74,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: CIRCLE_TIMES, "⊗",
	// }
}

func ExampleExpression_case75() {
	fmt.Println(exampleAST(77, "1.97 \\[CenterDot] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 75,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: CENTER_DOT, "·",
	// }
}

func ExampleExpression_case76() {
	fmt.Println(exampleAST(78, "1.97 \\[Star] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 76,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: STAR, "⋆",
	// }
}

func ExampleExpression_case77() {
	fmt.Println(exampleAST(79, "1.97 \\[VerticalTilde] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 77,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: VERTICAL_TILDE, "≀",
	// }
}

func ExampleExpression_case78() {
	fmt.Println(exampleAST(80, "1.97 \\[Coproduct] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 78,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: COPRODUCT, "∐",
	// }
}

func ExampleExpression_case79() {
	fmt.Println(exampleAST(81, "1.97 \\[Cap] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 79,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: CAP, "⌢",
	// }
}

func ExampleExpression_case80() {
	fmt.Println(exampleAST(82, "1.97 \\[Cup] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 80,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: CUP, "⌣",
	// }
}

func ExampleExpression_case81() {
	fmt.Println(exampleAST(83, "1.97 \\[CirclePlus] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 81,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: CIRCLE_PLUS, "⊕",
	// }
}

func ExampleExpression_case82() {
	fmt.Println(exampleAST(84, "1.97 \\[CircleMinus] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 82,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: CIRCLE_MINUS, "⊖",
	// }
}

func ExampleExpression_case83() {
	fmt.Println(exampleAST(85, "1.97 \\[Intersection] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 83,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: INTERSECTION, "⋂",
	// }
}

func ExampleExpression_case84() {
	fmt.Println(exampleAST(86, "1.97 \\[Union] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 84,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: UNION, "⋃",
	// }
}

func ExampleExpression_case85() {
	fmt.Println(exampleAST(87, ";;"))
	// Output:
	// &wl.Expression{
	// · Case: 85,
	// · Token: SPAN, ";;",
	// }
}

func ExampleExpression_case86() {
	fmt.Println(exampleAST(88, ";; 1.97"))
	// Output:
	// &wl.Expression{
	// · Case: 86,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: SPAN, ";;",
	// }
}

func ExampleExpression_case87() {
	fmt.Println(exampleAST(89, "1.97 ;;"))
	// Output:
	// &wl.Expression{
	// · Case: 87,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: SPAN, ";;",
	// }
}

func ExampleExpression_case88() {
	fmt.Println(exampleAST(90, "1.97 ;; 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 88,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: SPAN, ";;",
	// }
}

func ExampleExpression_case89() {
	fmt.Println(exampleAST(91, "1.97 \\[VerticalBar] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 89,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: VERTICAL_BAR, "\uf3d0",
	// }
}

func ExampleExpression_case90() {
	fmt.Println(exampleAST(92, "1.97 \\[NotVerticalBar] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 90,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: NOT_VERTICAL_BAR, "\uf3d1",
	// }
}

func ExampleExpression_case91() {
	fmt.Println(exampleAST(93, "1.97 \\[DoubleVerticalBar] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 91,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: DOUBLE_VERTICAL_BAR, "∥",
	// }
}

func ExampleExpression_case92() {
	fmt.Println(exampleAST(94, "1.97 \\[NotDoubleVerticalBar] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 92,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: NOT_DOUBLE_VERTICAL_BAR, "∦",
	// }
}

func ExampleExpression_case93() {
	fmt.Println(exampleAST(95, "1.97 \\[Element] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 93,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: ELEMENT, "∈",
	// }
}

func ExampleExpression_case94() {
	fmt.Println(exampleAST(96, "1.97 \\[NotElement] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 94,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: NOT_ELEMENT, "∉",
	// }
}

func ExampleExpression_case95() {
	fmt.Println(exampleAST(97, "1.97 \\[Subset] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 95,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: SUBSET, "⊂",
	// }
}

func ExampleExpression_case96() {
	fmt.Println(exampleAST(98, "1.97 \\[Superset] 1.98"))
	// Output:
	// &wl.Expression{
	// · Case: 96,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Expression2: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.98",
	// · · · },
	// · · },
	// · },
	// · Token: SUPERSET, "⊃",
	// }
}

func ExampleFactor() {
	fmt.Println(exampleAST(118, "1.97"))
	// Output:
	// &wl.Factor{
	// · Term: &wl.Term{
	// · · Case: 4,
	// · · Token: FLOAT, "1.97",
	// · },
	// }
}

func ExampleFactor_case1() {
	fmt.Println(exampleAST(119, "1.97 1.98"))
	// Output:
	// &wl.Factor{
	// · Case: 1,
	// · Factor: &wl.Factor{
	// · · Term: &wl.Term{
	// · · · Case: 4,
	// · · · Token: FLOAT, "1.98",
	// · · },
	// · },
	// · Term: &wl.Term{
	// · · Case: 4,
	// · · Token: FLOAT, "1.97",
	// · },
	// }
}

func ExampleTag() {
	fmt.Println(exampleAST(124, "a :: b"))
	// Output:
	// &wl.Tag{
	// · Token: IDENT, "b",
	// }
}

func ExampleTag_case1() {
	fmt.Println(exampleAST(125, "a :: \"b\""))
	// Output:
	// &wl.Tag{
	// · Case: 1,
	// · Token: STRING, "b",
	// }
}

func ExampleTerm() {
	fmt.Println(exampleAST(99, "<< \"a\""))
	// Output:
	// &wl.Term{
	// · Token: GET, "<<",
	// · Token2: STRING, "a",
	// }
}

func ExampleTerm_case01() {
	fmt.Println(exampleAST(100, "( 1.97 )"))
	// Output:
	// &wl.Term{
	// · Case: 1,
	// · Expression: &wl.Expression{
	// · · Case: 55,
	// · · Factor: &wl.Factor{
	// · · · Term: &wl.Term{
	// · · · · Case: 4,
	// · · · · Token: FLOAT, "1.97",
	// · · · },
	// · · },
	// · },
	// · Token: '(',
	// · Token2: ')',
	// }
}

func ExampleTerm_case02() {
	fmt.Println(exampleAST(101, "{ }"))
	// Output:
	// &wl.Term{
	// · Case: 2,
	// · Token: '{',
	// · Token2: '}',
	// }
}

func ExampleTerm_case03() {
	fmt.Println(exampleAST(102, "{ 1.97 }"))
	// Output:
	// &wl.Term{
	// · Case: 3,
	// · ExprList: &wl.ExprList{
	// · · Expression: &wl.Expression{
	// · · · Case: 55,
	// · · · Factor: &wl.Factor{
	// · · · · Term: &wl.Term{
	// · · · · · Case: 4,
	// · · · · · Token: FLOAT, "1.97",
	// · · · · },
	// · · · },
	// · · },
	// · },
	// · Token: '{',
	// · Token2: '}',
	// }
}

func ExampleTerm_case04() {
	fmt.Println(exampleAST(103, "1.97"))
	// Output:
	// &wl.Term{
	// · Case: 4,
	// · Token: FLOAT, "1.97",
	// }
}

func ExampleTerm_case05() {
	fmt.Println(exampleAST(104, "a"))
	// Output:
	// &wl.Term{
	// · Case: 5,
	// · Token: IDENT, "a",
	// }
}

func ExampleTerm_case06() {
	fmt.Println(exampleAST(105, "a :: b"))
	// Output:
	// &wl.Term{
	// · Case: 6,
	// · Tag: &wl.Tag{
	// · · Token: IDENT, "b",
	// · },
	// · Token: IDENT, "a",
	// · Token2: MESSAGE_NAME, "::",
	// }
}

func ExampleTerm_case07() {
	fmt.Println(exampleAST(106, "a :: b :: c"))
	// Output:
	// &wl.Term{
	// · Case: 7,
	// · Tag: &wl.Tag{
	// · · Token: IDENT, "b",
	// · },
	// · Tag2: &wl.Tag{
	// · · Token: IDENT, "c",
	// · },
	// · Token: IDENT, "a",
	// · Token2: MESSAGE_NAME, "::",
	// · Token3: MESSAGE_NAME, "::",
	// }
}

func ExampleTerm_case08() {
	fmt.Println(exampleAST(107, "97"))
	// Output:
	// &wl.Term{
	// · Case: 8,
	// · Token: INT, "97",
	// }
}

func ExampleTerm_case09() {
	fmt.Println(exampleAST(108, "a_"))
	// Output:
	// &wl.Term{
	// · Case: 9,
	// · Token: PATTERN, "a_",
	// }
}

func ExampleTerm_case10() {
	fmt.Println(exampleAST(109, "#97"))
	// Output:
	// &wl.Term{
	// · Case: 10,
	// · Token: SLOT, "#97",
	// }
}

func ExampleTerm_case11() {
	fmt.Println(exampleAST(110, "\"a\""))
	// Output:
	// &wl.Term{
	// · Case: 11,
	// · Token: STRING, "a",
	// }
}

func ExampleTerm_case12() {
	fmt.Println(exampleAST(111, "1.97 [[ 1.98 ]]"))
	// Output:
	// &wl.Term{
	// · Case: 12,
	// · ExprList: &wl.ExprList{
	// · · Expression: &wl.Expression{
	// · · · Case: 55,
	// · · · Factor: &wl.Factor{
	// · · · · Term: &wl.Term{
	// · · · · · Case: 4,
	// · · · · · Token: FLOAT, "1.98",
	// · · · · },
	// · · · },
	// · · },
	// · },
	// · Term: &wl.Term{
	// · · Case: 4,
	// · · Token: FLOAT, "1.97",
	// · },
	// · Token: LPART, "[[",
	// · Token2: RPART, "]]",
	// }
}

func ExampleTerm_case13() {
	fmt.Println(exampleAST(112, "1.97 !"))
	// Output:
	// &wl.Term{
	// · Case: 13,
	// · Term: &wl.Term{
	// · · Case: 4,
	// · · Token: FLOAT, "1.97",
	// · },
	// · Token: '!',
	// }
}

func ExampleTerm_case14() {
	fmt.Println(exampleAST(113, "1.97 ! !"))
	// Output:
	// &wl.Term{
	// · Case: 14,
	// · Term: &wl.Term{
	// · · Case: 4,
	// · · Token: FLOAT, "1.97",
	// · },
	// · Token: '!',
	// · Token2: '!',
	// }
}

func ExampleTerm_case15() {
	fmt.Println(exampleAST(114, "1.97 &"))
	// Output:
	// &wl.Term{
	// · Case: 15,
	// · Term: &wl.Term{
	// · · Case: 4,
	// · · Token: FLOAT, "1.97",
	// · },
	// · Token: '&',
	// }
}

func ExampleTerm_case16() {
	fmt.Println(exampleAST(115, "1.97 [ ]"))
	// Output:
	// &wl.Term{
	// · Case: 16,
	// · Term: &wl.Term{
	// · · Case: 4,
	// · · Token: FLOAT, "1.97",
	// · },
	// · Token: '[',
	// · Token2: ']',
	// }
}

func ExampleTerm_case17() {
	fmt.Println(exampleAST(116, "1.97 [ 1.98 ]"))
	// Output:
	// &wl.Term{
	// · Case: 17,
	// · ExprList: &wl.ExprList{
	// · · Expression: &wl.Expression{
	// · · · Case: 55,
	// · · · Factor: &wl.Factor{
	// · · · · Term: &wl.Term{
	// · · · · · Case: 4,
	// · · · · · Token: FLOAT, "1.98",
	// · · · · },
	// · · · },
	// · · },
	// · },
	// · Term: &wl.Term{
	// · · Case: 4,
	// · · Token: FLOAT, "1.97",
	// · },
	// · Token: '[',
	// · Token2: ']',
	// }
}

func ExampleTerm_case18() {
	fmt.Println(exampleAST(117, "1.97 '"))
	// Output:
	// &wl.Term{
	// · Case: 18,
	// · Term: &wl.Term{
	// · · Case: 4,
	// · · Token: FLOAT, "1.97",
	// · },
	// · Token: QUOTE, "'",
	// }
}

func Examplestart() {
	fmt.Println(exampleAST(1, "1.97"))
	// Output:
}
